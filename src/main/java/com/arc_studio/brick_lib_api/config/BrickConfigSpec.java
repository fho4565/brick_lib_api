...
public class BrickConfigSpec extends UnmodifiableConfigWrapper<UnmodifiableConfig> {
    ...
    private BrickConfigSpec(UnmodifiableConfig storage, UnmodifiableConfig values, Map<List<String>, String> levelComments, Map<List<String>, String> levelTranslationKeys) {
        super(storage);
        this.values = values;
    ...
    public synchronized boolean isCorrect(CommentedConfig config) {
        ...
        return correct(this.config, config, parentPath,
                Collections.unmodifiableList(parentPath), (a, b, c, d) -> {
                }, null, true) == 0;
    }
    ...
    public static class Builder {
        ...
        public BooleanValue define(List<String> path, Supplier<Boolean> defaultSupplier) {
            return new BooleanValue(this, define(path, defaultSupplier, o -> {
                if (o instanceof String) {
                    return "true".equalsIgnoreCase((String) o) || "false".equalsIgnoreCase((String) o);
                }
        ...
        public <T> Pair<T, BrickConfigSpec> configure(Function<Builder, T> consumer) {
            T o = consumer.apply(this);
            return Pair.of(o, this.build());
        }

        public BrickConfigSpec build() {
            context.ensureEmpty();
            ...
            BrickConfigSpec ret = new BrickConfigSpec(storage, valueCfg, levelComments, levelTranslationKeys);
    ...
    public static class ConfigValue<T> implements Supplier<T> {
         ...
         * @throws NullPointerException  if the {@link BrickConfigSpec config spec} object that will contain this has
         *                               not yet been built
